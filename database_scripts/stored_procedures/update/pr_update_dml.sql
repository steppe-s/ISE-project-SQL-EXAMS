...
CREATE OR ALTER PROCEDURE pr_update_dml
	@exam_db_name varchar(50),
	@dml_script varchar(MAX)
AS
BEGIN
	IF @exam_db_name IS NULL THROW 60004, 'exam_db_name cannot be null', 16;
		IF @dml_script IS NULL THROW 60031, 'dml_script cannot be null', 16;
    ...
    BEGIN TRY
        BEGIN TRANSACTION
			...
			UPDATE EXAM_DATABASE SET exam_dml_script = @dml_script WHERE exam_db_name = @exam_db_name
